...
var main = (function () {
	 ...
	 * @param {String} _enpoint Request to execute
	 * @param {Object} _params JSON object to send.
	 * @param {Function} _callback Function called once the method has been executed
	 * @return {void}
	 */
	function executeRequest(_enpoint, _params, _callback) {
		doLog && console.log(LOG_TAG, '- executeRequest: ' + _enpoint);
		var params = _params || {};
		...
		request({
			...
			if (_error) {
				...
				throw new Error('Execute Method: ' + _enpoint + '. Error: ' + JSON.stringify(_error, null, '\t'));
			}
