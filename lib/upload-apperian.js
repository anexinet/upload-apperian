...
var main = (function () {
	...
	function publishToApperian(_response, _fileId) {
		...
		executeMethod(METHOD_PUBLISH, params, enableApp);
	}
	 ...
	 * @return {void}
	 */
	function enableApp(_response) {
		doLog && console.log('Published successfully!');
		doLog && console.log(LOG_TAG, '- enableApp');
		var putOptions = {
			url: ENABLE_WEBSERVICE_URL + _response.result.appID,
			headers: {
			...
			if ((!filePath || filePath == '') && continueProcess) {
				...
				if (!fs.existsSync(filePath)) {
					...
					if (path.extname(filePath) != '.appx') {
						...
						doLog && console.log(LOG_TAG, 'App extension incorrect for Mircorosft');
						continueProcess = false;
					}
				}
			}
